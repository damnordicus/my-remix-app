...
export default function AddFilament () {
    ...
    const inputClass = 'w-5/6 p-2 border-2 py-2 text-lg rounded-md border-orange-500 bg-slate-800 shadow-md shadow-gray-700';
    ...
    return(
       ...
       <div className="flex-col mt-2  bg-slate-500/60 backdrop-blur-sm w-[350px] rounded-2xl shadow-lg border-2 border-slate-300">
        <div className="relative left-78 top-2 " ><XCircleIcon className="hover:cursor-pointer size-6" onClick={toggleView} /></div>
         ...
         <Form method="post" className="flex flex-col items-center space-y-3">
           ...
           <input className={inputClass} type="number" name="diameter" placeholder="Diameter (mm)" step="0.01" required defaultValue={1.75} />
           <input type="hidden" name="purchaseDate" value={new Date(Date.now()).toISOString()} />
     
           <button type="submit" name="_action" value="create" 
             className="w-5/6 mt-4 bg-amber-500 text-white p-2 mb-4 rounded-md hover:bg-amber-600 transition" >
             Add Filament
           </button>
         </Form>
       </div>
