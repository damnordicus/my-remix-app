...
export default function Register() {
    ...
    return (
        <div className="h-screen flex items-center justify-center">
            <div className="w-[300px] justify-center bg-slate-600/60 border-2 border-slate-500 backdrop-blur-xs rounded-xl flex flex-row flex-wrap items-center  gap-2 p-4">
                <h1 className="text-xl text-amber-400">Register New User</h1>
                <Form method="POST">
                    <div className="flex-col w-full text-lg gap-1 mb-2">
                        <label htmlFor="username">Username: </label>
                        <input type="text" className="bg-slate-800/60 rounded-xl border border-slate-400" name="username" required/>
                    </div>
                    <div className="flex-col w-full text-lg gap-1 mb-2">
                        <label htmlFor="email">Email: </label>
                        <input type="text" className="bg-slate-800/60 rounded-xl border border-slate-400" name="email" required/>
                    </div>
                    <div className="flex-col w-full text-lg gap-1 mb-2">
                        <label htmlFor="phone">Phone Number: </label>
                        <input type="text" className="bg-slate-800/60 rounded-xl border border-slate-400" name="phone" required/>
                    </div>
                    <div>
                        <button type="submit" name="_action" value="submit" className="bg-amber-600 text-black rounded-lg border-2 border-amber-400 px-2 w-full mt-6">Submit</button>
                    </div>
                </Form>
            </div>
        </div>
